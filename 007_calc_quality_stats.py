...
def quality_control(population, workspace):
    ...
    for subject in population:
        ...
        # Load data
        anat     = nb.load(os.path.join(subdir, 'ANATOMICAL', 'ANATOMICAL_BRAIN.nii.gz' )).get_data()
        anat_gm   = nb.load(os.path.join(subdir, 'ANATOMICAL', 'ANATOMICAL_GM.nii.gz' )).get_data()
        anat_wm   = nb.load(os.path.join(subdir, 'ANATOMICAL', 'ANATOMICAL_WM.nii.gz' )).get_data()
        anat_csf  = nb.load(os.path.join(subdir, 'ANATOMICAL', 'ANATOMICAL_CSF.nii.gz' )).get_data()
        ...
        anat_FWHM = fwhm(anat, anat_mask, out_vox=False)
        anat_QI1  = artifacts(anat, anat_mask)
        ...
        #
        # # EPI Ghosting
        # func_ghost = ghost_direction(nb.load(func_edit).get_data(), nb.load(func_mask).get_data(), direction="y")
        #
        #
        # print func_ghost
        #
        ...
        # df.loc[subject]['GSR'] = str(np.round(np.median(np.load('TSNR_data.npy')), 3))
