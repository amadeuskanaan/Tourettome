...
def concat_dataframes(population, workspace_dir, phenotypic_dir)
    ...
    df_cln = pd.concat([pd.read_csv(os.path.join(phenotypic_dir, 'CLINICAL/clinical_leipzig.csv'),index_col =0)
                        # pd.read_csv(os.path.join(phenotypic_dir, 'CLINICAL/clinical_paris.csv'), index_col=0)
                        # pd.read_csv(os.path.join(phenotypic_dir, 'CLINICAL/clinical_hannover_a.csv'), index_col=0)
                        # pd.read_csv(os.path.join(phenotypic_dir, 'CLINICAL/clinical_hannover_b.csv'), index_col=0)
                        # pd.read_csv(os.path.join(phenotypic_dir, 'CLINICAL/clinical_hamburg.csv'), index_col=0)
                        ])

    df_qc = [pd.read_csv(os.path.join(workspace_dir, subject, 'QUALITY_CONTROL/quality_paramters.csv'),
                         #index_col=0) for subject in df_cln]
                         index_col = 0) for subject in df_dcm.index]
