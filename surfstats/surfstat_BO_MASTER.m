...
for readcsv = 1
    ...
    cd(P)
    fid = fopen('/host/yeatman/local_raid/kanaan/workspace/Tourettome/phenotypic/phenotypic_tourettomex2.csv'); % final group
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Figure 2 analyses: compare Basal Ganglia Volumes

% Note that these should be based on MNI space volumes to control for ICV
for volume_analysis = 1    
    ...
    for b = 1:length(bg) 
        ...
        M        = 1 + SI + G ;
        slm      = SurfStatLinMod(seedk,M); 
        slm      = SurfStatT(slm,G.controls-G.patients); 
        disp(['simple volume: ' bg{b} ' ' num2str(slm.t,2)])
...
% here you want to make it based on native space volumes, as freesurfer is
% in native space as well 
for covariance = 1 
