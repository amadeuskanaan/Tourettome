...
# Forked from https://github.com/chrisfilo/alleninf

def fixed_effects(data, labels):
    corcoeff, p_val = pearsonr(data[labels[0]], data[labels[1]])
    print "Pearson correlation between %s and %s across all donors is %g (two tailed p value = %g)" % (
    labels[0], labels[1], corcoeff, p_val)
...
def bayesian_random_effects(data, labels, group, n_samples=2000, n_burnin=500):
    ...
    with pm.Model() as hierarchical_model:
        ...
        individual_intercepts = pm.Normal('individual intercepts', mu=group_intercept_mean, sd=group_intercept_variance,
                                          shape=n_donors)
    ...
    for i, c in enumerate(selection):
        ...
        for a_val, b_val in zip(hierarchical_trace['individual intercepts'][n_burnin::10][z],
                                hierarchical_trace['individual slopes'][n_burnin::10][z]):
            axis[i].plot(xvals, a_val + b_val * xvals, 'g', alpha=.1)
        axis[i].plot(xvals, hierarchical_trace['individual intercepts'][n_burnin:][z].mean() +
                     hierarchical_trace['individual slopes'][n_burnin:][z].mean() * xvals,
                     'g', alpha=1, lw=2.)
